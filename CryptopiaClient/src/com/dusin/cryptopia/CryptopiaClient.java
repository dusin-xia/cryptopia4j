...
import com.dusin.cryptopia.remote.*;
import com.dusin.cryptopia.remote.data.*;
...
public class CryptopiaClient {
    ...
    public CanceledTrade cancelTrade(TradeCancel cancel) {
        final String methodName = "CancelTrade";
        final JsonObject jo = new JsonObject();
        if (cancel.getOrderId() != null) {
            jo.addProperty("OrderId", cancel.getOrderId());
        }
        if (cancel.getTradePairId() != null) {
            jo.addProperty("TradePareId", cancel.getTradePairId());
        }
        jo.addProperty("Type", cancel.getType().getLabel());
        
        final String jsonResponse = privateApiQuery(methodName, jo.toString());
        final JsonElement jElement = new JsonParser().parse(jsonResponse);
        final JsonObject rootObject = jElement.getAsJsonObject();
        final ApiResponse<CanceledTrade> resp = new ApiResponse<>();
        resp.setJson(jsonResponse);
        resp.setSuccess(rootObject.get("Success").getAsBoolean());
        resp.setMessage(rootObject.get("Error").toString());
        validateResponse(resp);
        final JsonArray jsonArray = rootObject.get("Data").getAsJsonArray();
        final CanceledTrade trade = new CanceledTrade();
        trade.setOrderId(0L);
        trade.setData(new ArrayList<>());
        
        for (JsonElement arrayElement : jsonArray) {
            final JsonObject jFilledOrder = arrayElement.getAsJsonObject();
            trade.getData().add(jFilledOrder.getAsLong());
        }  
        return trade;
    }
    ...
    private List<OpenOrder> parseOpenOrders(String jsonResponse) {
        ...
        for (final JsonElement element : jsonArray) {
            ...
            String timeOrder = object.get("TimeStamp").toString();
            timeOrder = timeOrder.replace('T', ' ');
            timeOrder = timeOrder.substring(0, 20);
            //item.setTime(new Date(object.get("TimeStamp").getAsLong()));
